...
TaskProfiler Thread1Profiler, Thread2Profiler, DefaultThreadProfiler;
...
void Thread2Func(void const * argument)
{
  ...
  /* Infinite loop */
	...
	uint8_t message1[24] ="Thread1 Notified!\r\n";
  for(;;)
  {
	  ...
	  if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0) != GPIO_PIN_RESET){
		  ...
		  //HAL_UART_Transmit(&huart2,message,24,10);
		  xSemaphoreTake(uartMutexHandle,portMAX_DELAY);
		  HAL_UART_Transmit(&huart2,message1,24,10);
		  xSemaphoreGive(uartMutexHandle);
		  xTaskNotify(Thread1Handle,0x01,eSetBits);
